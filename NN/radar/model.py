...
class IstaLayerBefore(nn.Module):
    """
    One layer of the unrolled ISTA network. The radar here is applied BEFORE the sparse threshold operator.
    """
...
class IstaLayerAfter(nn.Module):
    """
    One layer of the unrolled ISTA network. The radar here is applied AFTER the sparse threshold operator.
    """
...
class IstaLayerIn(nn.Module):
    """
    One layer of the unrolled ISTA network. The radar here is applied IN the sparse threshold operator.
    """
...
class IstaNet(nn.Module):
        def __init__(self, shape, n_layers, radar_inclusion_type):
            super().__init__()
            ...
            # Model Selection
            if radar_inclusion_type == 'before':
                IstaLayer = IstaLayerBefore
                print('\nRadar before threshold')

            elif radar_inclusion_type == 'in':
                IstaLayer = IstaLayerIn
                print('\nRadar in threshold')

            elif radar_inclusion_type == 'after':
                IstaLayer = IstaLayerAfter
                print('\nRadar after threshold')

            else:
                raise Exception('Unrecognized Architecture. Try \'before\', \'in\', or \'after\'')
